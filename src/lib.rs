...
impl MerkleTreeWithHistory {
    ...
    pub fn insert(&mut self, leaf: U256) -> usize {
        ...
        return self.nextIndex as usize - 1;
    }
...
mod tests {
    ...
    fn test_merkletree_insert_single_3() {
        ...
        let expected = U256::from_str_radix("14817887234532324632578486942317778767513333548116388705259454362287888156301", 10).unwrap();
    ...
    fn test_merkletree_insert_single_16() {
        ...
        mt.insert(U256::new(5));
        let expected = U256::from_str_radix("20078220768011993253497856250024317483006104588209594787144509816521675548945", 10).unwrap();
    ...
    fn test_merkletree_insert() {
        let mut mt = MerkleTreeWithHistory::new(16);
        let expected = vec![
            U256::from_str_radix("3431256714363396804770991575090970055302175921802683225882378599453141462503", 10).unwrap(),
            U256::from_str_radix("7575821202546991722047889195143698024641067539407824397010939985717182566799", 10).unwrap(),
            U256::from_str_radix("7102419650151881575380791103194015368648640006236895399277475380346088306449", 10).unwrap(),
            U256::from_str_radix("3663265918960820756765744378616083555095944410653161772251208095179127101510", 10).unwrap(),
            U256::from_str_radix("15302658532613586889202868102641369060511299011842796454718345900410135644534", 10).unwrap(),
            U256::from_str_radix("19867311980617909474730049456052719869948526667934900087741729669853083711560", 10).unwrap(),
            U256::from_str_radix("6061878619835624285838818217971195365504071979555702464817484176105688178577", 10).unwrap(),
            U256::from_str_radix("2521963888311190328687829229664642120391801081246544527123137783093792814465", 10).unwrap(),
            U256::from_str_radix("10214875608306830392931189580024717263641319338206990452441323784791611321245", 10).unwrap(),
            U256::from_str_radix("7692234562883530752899755807890957688721742766928110244142163893445927985263", 10).unwrap(),
        ];
