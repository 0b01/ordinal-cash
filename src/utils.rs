...
pub fn mod_pow(mut base: U256, mut exp: U256, modulus: U256) -> U256 {
    if modulus == 1 { return U256::new(0) }
...
mod tests {
    ...
    fn test_addmod_under_10() {
        let a = U256::new(1); let b = U256::new(2); let m = U256::new(10);
        assert_eq!(U256::new(3), addmod(a, b, m));
    }

    #[test]
    fn test_addmod_over_10() {
        let a = U256::new(9); let b = U256::new(9); let m = U256::new(10);
        assert_eq!(U256::new(8), addmod(a, b, m));
    }

    #[test]
    fn test_addmod_overflow() {
        let a = U256::from_str_radix("21663839004416932945382355908790599225266501822907911457504978515578255421292", 10).unwrap();
        let m = U256::from_str_radix("21888242871839275222246405745257275088548364400416034343698204186575808495617", 10).unwrap();
        let b = m - a + 1;
    ...
    #[test]
    fn test_pow_mod() {
        let b = U256::from_str_radix("7120861356467848435263064379192047478074060781135320967663101236819528304087", 10).unwrap();
        let e = U256::from_str_radix("5", 10).unwrap();
        let m = U256::from_str_radix("21888242871839275222246405745257275088548364400416034343698204186575808495617", 10).unwrap();
        let expected = U256::from_str_radix("10745147226606258107230518846003648962088186972186954460246329063381405757050", 10).unwrap();
